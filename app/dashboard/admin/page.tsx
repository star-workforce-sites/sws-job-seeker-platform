...
export default async function AdminDashboardPage() {
  ...
  try {
    const subResult = await sql`
      SELECT
        s.id AS subscription_id, s.subscription_type AS plan_name,
        s.status AS subscription_status, s.created_at AS subscribed_at,
        u.id AS user_id, u.name AS job_seeker_name, u.email AS job_seeker_email,
      ...
      LEFT JOIN recruiter_assignments ra ON ra.subscription_id = s.id AND ra.status = 'active'
      LEFT JOIN users r ON r.id = ra.recruiter_id
      WHERE s.status = 'active' AND s.subscription_type LIKE 'recruiter_%'
      ORDER BY s.created_at DESC
    `
    subscribers = subResult.rows
  } catch (error) {
    console.error("[AdminDashboard] subscribers error:", error)
  }

  // ── Fetch recruiters with assignment counts ──────────────
  let recruiters: any[] = []
  try {
    const recResult = await sql`
      SELECT
        u.id, u.name, u.email, u.suspended,
    ...
    console.error("[AdminDashboard] recruiters error:", error)
  }
