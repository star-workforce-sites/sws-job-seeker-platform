...
/** Admin notification recipients — single source of truth */
export const ADMIN_EMAILS = ['Srikanth@startekk.net', 'info@startekk.net']
...
export async function sendPurchaseNotificationEmail(params: {
  ...
  try {
    ...
    // Build extra rows for resume distribution targeting info (escaped for XSS safety)
    const extraRows = [
      metadata.targetRoles && `<tr><td style="padding:6px 0;color:#666;width:160px;">Target Roles:</td><td style="padding:6px 0;font-weight:500;">${esc(metadata.targetRoles)}</td></tr>`,
      metadata.targetLocations && `<tr><td style="padding:6px 0;color:#666;">Locations:</td><td style="padding:6px 0;font-weight:500;">${esc(metadata.targetLocations)}</td></tr>`,
      metadata.industry && `<tr><td style="padding:6px 0;color:#666;">Industry:</td><td style="padding:6px 0;">${esc(metadata.industry)}</td></tr>`,
      metadata.experience && `<tr><td style="padding:6px 0;color:#666;">Experience:</td><td style="padding:6px 0;">${esc(metadata.experience)} yrs</td></tr>`,
    ].filter(Boolean).join('\n')
      ...
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        ...
        <div style="padding:20px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
          ...
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:6px 0;color:#666;width:160px;">Customer:</td><td style="padding:6px 0;font-weight:bold;">${esc(customerName)}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Email:</td><td style="padding:6px 0;"><a href="mailto:${esc(customerEmail)}" style="color:#0A1A2F;">${esc(customerEmail)}</a></td></tr>
            <tr><td style="padding:6px 0;color:#666;">Product:</td><td style="padding:6px 0;font-weight:bold;color:#059669;">${esc(productName)}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Amount:</td><td style="padding:6px 0;font-weight:bold;font-size:16px;">$${esc(amount)}</td></tr>
          ...
          <div style="margin-top:16px;padding:12px;background:#fef3c7;border-radius:6px;border-left:4px solid #f59e0b;">
            <p style="margin:0;font-size:13px;color:#92400e;">
              <strong>Action needed:</strong> Manually fulfill the resume distribution for ${esc(customerEmail)}
  ...
  try {
    ...
    const seekerHtml = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        ...
        <div style="padding:20px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
          <p style="margin:0 0 16px;color:#374151;">Hi ${esc(jobSeekerName) || 'there'},</p>
          <p style="margin:0 0 16px;color:#374151;">Your recruiter <strong>${esc(recruiterName)}</strong> just submitted an application on your behalf:</p>
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:6px 0;color:#666;width:120px;">Position:</td><td style="padding:6px 0;font-weight:bold;">${esc(jobTitle)}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Company:</td><td style="padding:6px 0;font-weight:bold;">${esc(companyName)}</td></tr>
            ${jobUrl ? `<tr><td style="padding:6px 0;color:#666;">Job Link:</td><td style="padding:6px 0;"><a href="${esc(jobUrl)}" style="color:#0A1A2F;">View Job Posting</a></td></tr>` : ''}
    ...
    const adminHtml = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        ...
        <div style="padding:20px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
          ...
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:6px 0;color:#666;width:120px;">Recruiter:</td><td style="padding:6px 0;font-weight:bold;">${esc(recruiterName)}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Client:</td><td style="padding:6px 0;font-weight:bold;">${esc(jobSeekerName) || 'N/A'} (${esc(jobSeekerEmail)})</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Position:</td><td style="padding:6px 0;">${esc(jobTitle)}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Company:</td><td style="padding:6px 0;">${esc(companyName)}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Time:</td><td style="padding:6px 0;">${new Date().toLocaleString('en-US', { timeZone: 'America/Chicago', dateStyle: 'full', timeStyle: 'short' })} CT</td></tr>
          </table>
        </div>
      </div>
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        ...
        <div style="padding:20px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
          ...
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:6px 0;color:#666;width:140px;">Name:</td><td style="padding:6px 0;font-weight:bold;">${esc(jobSeekerName) || 'Not set'}</td></tr>
            <tr><td style="padding:6px 0;color:#666;">Email:</td><td style="padding:6px 0;"><a href="mailto:${esc(jobSeekerEmail)}" style="color:#0A1A2F;">${esc(jobSeekerEmail)}</a></td></tr>
            <tr><td style="padding:6px 0;color:#666;">Time:</td><td style="padding:6px 0;">${new Date().toLocaleString('en-US', { timeZone: 'America/Chicago', dateStyle: 'full', timeStyle: 'short' })} CT</td></tr>
          </table>
